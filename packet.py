...
class Packet(object):
    ...
    def read_eth_header(self, data):
        ...
        eth_dst, eth_src, eth_type = struct.unpack('! 6s 6s 2s', data[:14])
        return self.get_hr_mac(eth_dst), self.get_hr_mac(eth_src), eth_type.hex(), data[14:]
    ...
    def read_ipv4_header(self, data):
        ...
        return ip_proto, self.get_hr_ipv4(ip_src), self.get_hr_ipv4(ip_dst), data[20:]        
    
    def read(self, data):
        eth_dst, eth_src, eth_type, eth_payload = self.read_eth_header(data)
        if eth_type != '0800': #только IPv4 EtherTypes
            return
